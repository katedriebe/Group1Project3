...
class ModifiedResNet50(nn.Module):
    def __init__(self, num_classes=12):
        ...
        # Adjusting deprecated usage as per warning
...
# Set the directory where your Hong Kong protest images are stored
...
print(f"Data loader setup with batch size {data_loader.batch_size}. Total batches to process: {len(data_loader)}.")

# Load the model
model = ModifiedResNet50(num_classes=12)
model.to(device).eval()  # Set the model to evaluation mode

# Evaluate the images
results = []
with torch.no_grad():
    for inputs, img_names in tqdm(data_loader, desc="Evaluating", total=len(data_loader)):
...
output_csv_path = os.path.join(img_dir, 'hongkong_protest_results.csv')
